...
### 上传文件
您可以点击“上传文件（Click to Upload)"按钮上传文件，上传成功的文件将会显示在Notebook的file中。若上传的文件为压缩包，则输入并运行代码!unzip -o file_name.zip 进行解压
...
当模型训练完成后，你也可以在自己的移动和嵌入式设备中运行你的模型。

#### 1. 保存模型为 SavedModel 或 tf.keras格式

   如果你在 Tensorflow 框架下构建并训练你的模型，你需要将模型保存为SavedModel格式。SavedModel 是一种独立于语言且可恢复的序列化格式，使较高级别的系统和工具可以创建、使用和转换 TensorFlow 模型。
...
更多关于SavedModel的信息请见[官方文档](https://www.tensorflow.org/programmers_guide/saved_model)
   ...
   当转换为TensorFlow Lite格式时，需要将模型和权重都保存在同一个文件中。如果你在 Tensorflow Keras 框架下构建并训练你的模型，你可以使用```tf.keras.models.save_model```函数来保存你的模型。目前，Tensorflow Keras 公开了很多预训练的模型，例如VGG、ResNet等。但有时，你获得的只是模型的预训练权重文件。这时，你可以这样来处理，例如
...
如果你的模型保存为 SavedModel 格式，那么你将会有一个包含一系列子文件的文件夹。右键点击此文件夹，选择 convert to tensorlite即可。

如果你的模型实在tf.keras框架下构建并训练的，那么你将会有一个 ```.h5``` 为后缀的模型文件。右键点击此文件，选择 convert to tensorlite即可。

目前，我们只支持 SavedModel 或 tf.keras 格式下简单模型的自动转换，更多的功能正在添加中。如果你的模型复杂或者你需要转换其他格式的模型文件，请参阅 tensorflow lite convert的[官方文档](https://www.tensorflow.org/lite/convert/python_api)进行手动转换。

#### 3. 下载转换后的模型并将其嵌入你的本地程序

模型转换完成后，在左侧的文件列表中便可以看到以转换后的以 ```.tflite``` 为后缀的文件。右键此文件，并点击下载，即可将其下载到你的本地电脑中。然后你可以将其嵌入到你的 Android app、iOS app 或 Raspberry Pi中。更多信息请参阅[tensorflow官方文档](https://www.tensorflow.org/lite/devguide)
