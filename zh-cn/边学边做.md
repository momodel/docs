...
### 1. 通过模版的形式创建项目
你可以通过新建应用项目，选择从[模版中创建一个教程项目](https://momodel.github.io/docs/#/zh-cn/%E5%B9%B3%E5%8F%B0%E6%95%99%E7%A8%8B?id=%E5%88%9B%E5%BB%BA%E6%95%99%E7%A8%8Bcreate-project)，按照项目中的`.ipynb`文件指引边学边做，熟悉Mo使用方法和开发流程。
...
### 1. 新建数据集项目
数据集是由数据组成的集合，是模型和应用开发的基础材料。该部分将指引你从创建数据集项目开始，学习上传和发布一个新的数据集。你可以通过新建应用项目，选择从[模版中创建一个教程项目](https://momodel.github.io/docs/#/zh-cn/%E5%B9%B3%E5%8F%B0%E6%95%99%E7%A8%8B?id=%E5%88%9B%E5%BB%BA%E6%95%99%E7%A8%8Bcreate-project)选择该教程进行实践。
...
`工具模块(Module-Toolkit)`是一个不需要二次训练的模块，本部分我们将引导你完成一个将图片的 base64 字符串还原为图片的 Toolkit，该模块是开发图片应用常用的模块(Module)组件，你可以通过新建模块项目，选择从[模版中创建和复制该项目](https://momodel.github.io/docs/#/zh-cn/%E5%B9%B3%E5%8F%B0%E6%95%99%E7%A8%8B?id=%E5%88%9B%E5%BB%BA%E6%95%99%E7%A8%8Bcreate-project)，然后打开工程中的 Notebook 文件依次运行其中的Cell代码，完成相应的操作。
...
<img src='https://ws3.sinaimg.cn/large/006tNbRwly1fylil7qfi5j30ie0rkgow.jpg' width=40% height=40%>
...
### 1. 简介
你已经学习了[开发一个工具模块(Module-Toolkit)](https://momodel.github.io/docs/#/zh-cn/%E8%BE%B9%E5%AD%A6%E8%BE%B9%E5%81%9A?id=%E5%BC%80%E5%8F%91%E4%B8%80%E4%B8%AA%E5%B7%A5%E5%85%B7%E6%A8%A1%E5%9D%97module-toolkit)的教程，`模型模块(Module-Model)`是可以供开发者二次训练的模块。本部分以SKlearn中的kmeans聚类算法为例指引你在Mo平台上开发部署一个可二次训练的模块(Module-Model)。你可以通过新建模块项目，选择从[模版中创建和复制该项目](https://momodel.github.io/docs/#/zh-cn/%E5%B9%B3%E5%8F%B0%E6%95%99%E7%A8%8B?id=%E5%88%9B%E5%BB%BA%E6%95%99%E7%A8%8Bcreate-project)，打开工程中的 Notebook 依次运行其中的Cell代码，完成相应的操作。
...
本教程将指引如何在开发你的应用(app)时使用模块(module)和数据集(dataset)。

## 1. 新建一个Notebook
您新建的任何项目的 notebook 都默认有下面这个cell代码，这部分代码的功能是引入一些默认的包、Client和其他一些控制文件。不同的用户和项目下 Client 中的内容不同。

![](https://ws4.sinaimg.cn/large/006tKfTcly1g0gd3ya80wj31560qctdu.jpg)
...
在 Mo 的 Notebook 中， 我们可以轻松的在我们自己的代码中， 插入别人已经编写好的模块 (我们也称之为Module)。  模块(Module) 根据是否可以二次训练，可进一步分为可二次训练的模型(model)和不可二次训练的工具(toolkit)。我们先看看如何插入一个工具 (toolkit)。
...
<img src='https://ws4.sinaimg.cn/large/006tNbRwly1fwq0gaefc3j30rs0640tf.jpg' width='50%'>

## 4. 二次训练模型

在前面的教程中，我们已经了解到，模块分为两类，不需要二次训练的工具模块和可以二次训练的模型模块。接下来，我们看如何对引入的模型模块进行二次训练。
...
9. 训练完成后，在左侧可以看到模型和权重文件已经被保存下来了，然后我们就可以在应用中像 toolkit 一样使用它了。
...
本部分将指引你通过调用别人已经部署的模块(Module)，帮组你熟悉开发和部署一个应用的流程。你可以在新建应用项目时选择[从模版创建该教程项目](https://momodel.github.io/docs/#/zh-cn/%E5%B9%B3%E5%8F%B0%E6%95%99%E7%A8%8B?id=%E5%88%9B%E5%BB%BA%E6%95%99%E7%A8%8Bcreate-project)，打开工程下的Notebook，按指引完成即可。当然你也可以通过新建一个空的项目，按以下指引熟悉开发和部署一个应用的流程。
...
### 4. 二次训练他人发布公开的模型模块
在前面的教程中，我们已经了解到,模块分为两类,不需要二次训练的工具模块和可以二次训练的模型模块.我们也学会了如何开发他们。我们也同样学会了如何在开发应用的时候,如何引入并使用它们。接下来,我们看如何对引入的模型模块进行二次训练.
