...
### 1. 简介
你已经学习了[开发一个可预测的模块(Module-Toolkit)]()的教程，并且已经明确了Toolkit 和 Model 的区别。本部分以SKlearn中的kmeans聚类算法为例指引你在Mo平台上开发部署一个可训练的模块(Module-Model)。你可以通过新建模块项目，选择从模版中创建和复制该项目，打开工程中的 Notebook 依次运行其中的Cell代码，完成相应的操作。
