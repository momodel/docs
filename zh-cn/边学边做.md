...
### 1. 新建数据集项目
数据集是由数据组成的集合，是模型和应用开发的基础材料。该部分将指引你从创建数据集项目开始，学习上传和发布一个新的数据集。你可以在训练营中的[平台功能教程](http://www.momodel.cn:8899/#/classroom/class?id=5c5696cd1afd9458d456bf54&type=doc)，选择`上传发布一个数据集`，来进行实践。
...
当你需要将数据集插入当前项目的时候,首先打开 一个 Notebook(.ipynb) 文件，然后在数据集页面点击 Import 按钮，此数据集将以只读模式自动挂载到/datasets/<dataset name>
...
4.2 导入 `main` 函数
打开本教程 Notebook，选择下面提示插入 `main` 函数的代码块，点击左侧的 Deploy 按钮，选择第一步的 插入。
