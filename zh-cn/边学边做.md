...
> * 在Mo上运行你的第一段代码
> * 上传发布一个数据集
> * 开发一个不需要训练的模块(Module-Toolkit)
> * 开发一个可二次训练的模块(Module-Model)
> * 开发和部署一个应用(APP)
> * 在GPU/CPU资源上训练机器学习模型
...
本部分我们将引导你完成一个将图片的 base64 字符串还原为图片的 Toolkit，该模块是开发图片应用常用的模块(Module)组件，你可以通过新建模块项目，选择从模版中创建和复制该项目，然后打开工程中的 Notebook 文件依次运行其中的Cell代码，完成相应的操作。
...
### 1. 简介
你已经学习了[开发一个不需要训练的模块(Module-Toolkit)]()的教程，并且已经明确了Toolkit 和 Model 的区别。本部分以SKlearn中的kmeans聚类算法为例指引你在Mo平台上开发部署一个可二次训练的模块(Module-Model)。你可以通过新建模块项目，选择从模版中创建和复制该项目，打开工程中的 Notebook 依次运行其中的Cell代码，完成相应的操作。
