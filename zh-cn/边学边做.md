...
双击 main.py 文件，可以看到，其中的主要的方法有 train，predict，和 load_model。其中类名是根据项目名称生成的，如果您的类名和项目名称不同请修改。

<img src='https://ws4.sinaimg.cn/large/006tNbRwly1fxtl3jb99qj31bb0u0wlb.jpg' width=80% height=80%>

在部署的时候，train和predict方法是必须要补充完成的，在您部署成功并发布后，可以调用train来训练模型，调用predict来使用模型进行预测。
...
整理好 handle 函数之后，点击第二部的 start 按钮，开始准备部署时需要的文件。
...
在部署的时候，系统会自动对 handler.py 文件和 YML 配置文件进行基本的格式检查，检查通过后才能进行部署，您可以选择需要发布的文件或勾选发布dev版或正式版本（dev为测试版本，部署后只有所有者可以使用，正式版项目为公开的，所有人可见），然后点击 `OK`，进行部署。

<img src='https://ws2.sinaimg.cn/large/006tNbRwly1fy2rhz1763j30si0saacu.jpg' width=50% height=50%>

在短暂的等待后, 回到应用的详情页面, 在右上角会有部署成功的通知。恭喜您！您已经完成了您的第一个APP的部署。

### 5. 运行已部署的应用
然后您就可以找到这个项目，在网页中输入参数，运行得到输出结果。
...
您可以使用TensorBoard来可视化您的机器学习模型的推理图（graph）结构，显示训练过程中准确率和损失函数的变化，绘制关于图形执行的量化指标等等。下面以手写数据集MNIST为例，来帮助您在Mo平台上更便捷的使用 TensorBoard 可视化工具。更多关于TensorBoard 的信息请参考[官方文档](https://www.tensorflow.org/programmers_guide/summaries_and_tensorboard) 和 [官方Github开源项目](https://github.com/tensorflow/tensorboard)
