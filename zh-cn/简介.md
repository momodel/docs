...
Mo是一个面向个人用户与企业，让大众能够开发和使用`人工智能应用的平台`。它建立并提供一个能够将需求发布者、模块开发与组装者、应用使用者汇聚起来的平台环境。Mo始于数据，忠于用户，致力于`降低AI技术使用门槛、缩短学习曲线`，是一个为实现`人工智能民主化、应用普及化`目标而生的交互式线上数据模型设计、开发、应用平台。
